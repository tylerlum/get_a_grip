...
def _compute_fingertip_keyword_to_contact_candidates(
    link_name_to_contact_candidates: Dict[str, torch.Tensor],
    fingertip_keywords: List[str],
) -> Dict[str, torch.Tensor]:
    # Merge links associated with same fingertip
    fingertip_keyword_to_contact_candidates = {}
    for fingertip_keyword in fingertip_keywords:
        ...
        fingertip_keyword_to_contact_candidates[fingertip_keyword] = torch.cat(
            merged_contact_candidates, dim=1
        )
    return fingertip_keyword_to_contact_candidates
...
def compute_fingertip_mean_contact_positions(
    ...
    fingertip_keyword_to_contact_candidates = (
        _compute_fingertip_keyword_to_contact_candidates(
            link_name_to_contact_candidates=link_name_to_contact_candidates,
            fingertip_keywords=hand_model.fingertip_keywords,
        )
    )
    ...
    for i, fingertip_keyword in enumerate(fingertip_keywords):
        contact_candidates = fingertip_keyword_to_contact_candidates[fingertip_keyword]
...
def compute_closest_contact_point_info(
    ...
    # To be populated
    num_fingers = len(fingertip_keyword_to_contact_candidates)
    batch_size = joint_angles.shape[0]
...
def compute_fingertip_dirs(
    ...
    current_status = hand_model.chain.forward_kinematics(joint_angles)
    fingertip_link_names = set(hand_model.fingertip_names)
...
def compute_grasp_orientations_from_z_dirs(
    ...
    option_1_ok = (
        torch.linalg.cross(center_to_tip_dirs, z_dirs).norm(dim=-1, keepdim=True) > 1e-4
    )
    ...
    x_dirs = torch.linalg.cross(y_dirs, z_dirs, dim=-1)
