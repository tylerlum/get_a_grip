...
CAM_POS = CAM_TARGET + gymapi.Vec3(0.3, 0.3, 0.0)
ISAAC_DATETIME_STR = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")


# BRITTLE: Hardcoded virtual joint names to match the urdf
VIRTUAL_JOINT_NAMES = [
...
class IsaacValidator:
    def __init__(
        ...
        if hand_model_type == HandModelType.ALLEGRO:
            ...
            allowed_contact_link_names = ALLEGRO_HAND_ALLOWED_CONTACT_LINK_NAMES
            hand_file = ALLEGRO_HAND_FILE_WITH_VIRTUAL_JOINTS
        elif hand_model_type == HandModelType.LEAP:
            joint_names = LEAP_HAND_JOINT_NAMES
            allowed_contact_link_names = LEAP_HAND_ALLOWED_CONTACT_LINK_NAMES
            hand_file = LEAP_HAND_FILE_WITH_VIRTUAL_JOINTS
        else:
            raise ValueError(f"Unknown hand_model_type: {hand_model_type}")

        self.joint_names = joint_names
        self.allowed_contact_link_names = allowed_contact_link_names
        ...
        self.table_root = str(get_assets_folder())
        self.table_file = "table/table.urdf"
        ...
        self.sim_params.physx.max_gpu_contact_pairs = (
            8 * 1024 * 1024
        )  # Default is 1024 * 1024, but we have more contacts. For some extreme objects, need to increase this further to e.g. 32 * 1024 * 1024
        ...
        self.table_asset = gym.load_asset(
            self.sim, self.table_root, self.table_file, self.table_asset_options
        )
        self.table_texture = gym.create_texture_from_file(
            self.sim, str(get_assets_folder() / "table/wood.png")
        )

        self.hand_asset_dof_names = gym.get_asset_dof_names(self.hand_asset)
        assert set(VIRTUAL_JOINT_NAMES + self.joint_names) == set(
            self.hand_asset_dof_names
        ), (
            f"Only in VIRTUAL_JOINT_NAMES + self.joint_names: {set(VIRTUAL_JOINT_NAMES + self.joint_names) - set(self.hand_asset_dof_names)}\n"
            + f"Only in self.hand_asset_dof_names: {set(self.hand_asset_dof_names) - set(VIRTUAL_JOINT_NAMES + self.joint_names)}"
        )
    ...
    def _compute_init_obj_pose_above_table(self, obj_scale: float) -> gymapi.Transform:
        ...
        if USE_HARDCODED_MAX_EXTENT:
            # In this method, all objs are assumed to be centered in a bounding box, with the max width being 2.0m (unscaled)
            ...
            # Read in object mesh, use bounding box to place the object above the table with small gap
    ...
    def _setup_hand(
        ...
        # Virtual joints
        for joint in VIRTUAL_JOINT_NAMES:
            joint_idx = gym.find_actor_dof_index(
                env, hand_actor_handle, joint, gymapi.DOMAIN_ACTOR
            )
    ...
    def _run_sim_steps(self) -> Tuple[List[bool], List[bool], np.ndarray]:
        ...
        while sim_step_idx < self.num_sim_steps:
            ...
            if not self.is_paused:
                ...
                if self.camera_handles and sim_step_idx > 0:
                    ...
                    for ii, env in enumerate(self.camera_envs):
                        ...
                        self.env_to_video_frames[ii].append(video_frame)
            ...
            if self.has_viewer:
                ...
                desc = default_desc
                if self.is_step_mode:
                    desc += ". STEP MODE: Press SPACE to take 1 sim step"
                else:
                    desc += ". NORMAL MODE: Press SPACE to toggle pause"
                desc += ". Press S to toggle step mode"
                if self.is_paused:
                    desc += ". (Paused)"
                pbar.set_description(desc)

        assert object_states_before_grasp is not None
    ...
    def _get_actor_indices(self, envs, actors) -> torch.Tensor:
        ...
        actor_indices = torch_utils.to_torch(
            [
                gym.get_actor_index(env, actor, gymapi.DOMAIN_SIM)
    ...
    def _save_video_if_needed(self) -> None:
        if self.camera_handles:
            for ii, _ in enumerate(self.camera_envs):
                ...
                print(
                    f"Rendering camera {ii} to video at path {video_path} with {len(self.env_to_video_frames[ii])} frames."
                )
                self._render_video(
                    video_frames=self.env_to_video_frames[ii],
    ...
    def _compute_virtual_joint_dof_pos_targets(
        ...
        assert len(VIRTUAL_JOINT_NAMES) == 6
    ...
    def _set_virtual_joint_dof_pos_targets(
        ...
        for env, hand_handle, dof_pos_target in zip(
            ...
            for i, joint in enumerate(VIRTUAL_JOINT_NAMES):
                joint_idx = gym.find_actor_dof_index(
    ...
    def reset_simulator(self) -> None:
        ...
        # Recreate hand and table asset in new sim.
        self.hand_asset = gym.load_asset(
            self.sim, self.hand_root, self.hand_file, self.hand_asset_options
        )
        self.table_asset = gym.load_asset(
    ...
    def _init_or_reset_state(self) -> None:
        ...
        self.env_to_video_frames = defaultdict(list)
        self.obj_asset = None
        self.obj_root = None
        self.obj_file = None
    ...
    def _save_single_image_lightweight(
        ...
        if generate_depth:
            ...
            if numpy_depth:
                np.save(file=path / f"dep_{ii}.npy", arr=depth_image)
            else:
