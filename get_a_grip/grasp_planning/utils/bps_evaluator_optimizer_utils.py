...
class RandomSamplingOptimizer:
    def __init__(
        self,
        bps_evaluator: BpsEvaluatorModel,
        bps: torch.Tensor,
        init_grasps: torch.Tensor,
    ) -> None:
    ...
    def step(self) -> torch.Tensor:
        with torch.no_grad():
            old_losses = 1 - self.bps_evaluator(f_O=self.bps, g_O=self.grasps)[:, -1]
            # print(f"Old grasps: {self.grasps}")

            new_grasps = self.add_noise(self.grasps)

            new_losses = 1 - self.bps_evaluator(f_O=self.bps, g_O=new_grasps)[:, -1]
...
def main() -> None:
    ...
    bps_evaluator = BpsEvaluatorModel(in_grasp=37, in_bps=4096).to(device)
    bps_evaluator.eval()
    ...
    random_samping_optimizer = RandomSamplingOptimizer(
        bps_evaluator=bps_evaluator, bps=bps, init_grasps=init_grasps
    )
