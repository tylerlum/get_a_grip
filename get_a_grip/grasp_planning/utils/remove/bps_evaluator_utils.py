...
from get_a_grip.grasp_planning.utils.bps_evaluator_optimizer_utils import (
...
def get_optimized_grasps(
    ...
    bps_evaluator = BpsEvaluatorModel(in_grasp=3 + 6 + 16 + 12, in_bps=N_BASIS_PTS).to(
        device
    )
    bps_evaluator.eval()

    if ckpt_path.exists():
        bps_evaluator.load_state_dict(torch.load(ckpt_path, map_location=device))
    else:
    ...
    with torch.no_grad():
        ...
        for batch_i in tqdm(
            ...
            success_preds = (
                bps_evaluator(f_O=f_O, g_O=g_O)[:, -1].detach().cpu().numpy()
            )
            assert (
...
def run_bps_evaluator_sim_eval(args: CommandlineArgs) -> None:
    print("=" * 80)
    print(f"args: {args}")
    print("=" * 80 + "\n")
...
def main() -> None:
    ...
    run_bps_evaluator_sim_eval(args)
