...
# %% [markdown]
# # Create NeRF Grasp Dataset
#
# The purpose of this script is to iterate through each NeRF object and evaled grasp config, sample densities in a fixed grid and along the fingertip trajectories, and store the data
...
from get_a_grip.grasp_planning.utils.optimizer_utils import AllegroGraspConfig
...
experiment_folders = sorted(
    list(pathlib.Path("data").glob("2024-05-06_rotated_stable_grasps_*"))
    + list(pathlib.Path("data").glob("2024-05-26_rotated_v2_only_grasps_*"))
)
...
with h5py.File(cfg.output_filepath, "w") as hdf5_file:
    ...
    for object_i, evaled_grasp_config_dict_filepath in pbar:
        ...
        try:
            ...
            y_colls = evaled_grasp_config_dict[
            ...
            if (
                ...
                y_PGSs = y_PGSs[: cfg.max_num_data_points_per_file]
                y_picks = y_picks[: cfg.max_num_data_points_per_file]
                y_colls = y_colls[: cfg.max_num_data_points_per_file]
            ...
            if isinstance(cfg, GridNerfDataConfig):
                ...
                if N_FRAME_ORIGIN == "table":
                    ...
                    X_N_Oy[
                        1, 3
                    ] = -min_y  # Eg. min_y=-0.1, then this means that the object is 0.1m above the table
...
if "nerf_densities" in globals():
    ...
    fig = plot_mesh_and_query_points(
        ...
        title=f"Mesh and Query Points, Success: {y_PGSs[cfg.grasp_visualize_index]}",
    )
    fig.show()
...
fig2 = plot_mesh_and_transforms(
    ...
    num_fingers=cfg.fingertip_config.n_fingers,
    title=f"Mesh and Transforms, Success: {y_PGSs[cfg.grasp_visualize_index]}",
)
fig2.show()
